/* StyleCraft v1.5.0 — Options Page */
(async function(){
  ...
  function exportSingleDomain(domain) {
    ...
    const exp = { domain, data, version: '1.5.0', exported: new Date().toISOString() };
  ...
  $('btn-export').addEventListener('click',()=>{
    const exp={data:allData,settings,version:'1.5.0',exported:new Date().toISOString()};
    const blob=new Blob([JSON.stringify(exp,null,2)],{type:'application/json'});
